...
class Padding {
    ...
    //Mengubah padding ketika total kata tidak kelipatan 16 maka akan ditambahkan spasi
...
//            return output;
            ...
            String SaveFileTemp = new String(plaintextBytes);
            int Length = SaveFileTemp.length() + (16-(SaveFileTemp.length() % 16));
            char [] SaveCharTemp = new char[Length];
            for(int i = 0 ; i < Length ; i++){
                if(SaveFileTemp.length() > i){
                    SaveCharTemp[i] = SaveFileTemp.charAt(i);
                }else{
                    SaveCharTemp[i] = ' ';
                }
            }
            String CharToStr = String.valueOf(SaveCharTemp);
            
            return CharToStr.getBytes();
        }
    }
